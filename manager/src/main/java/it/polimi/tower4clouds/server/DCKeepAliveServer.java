...
public class DCKeepAliveServer extends ServerResource {

	private static final Logger logger = LoggerFactory
			.getLogger(DCKeepAliveServer.class);
	...
	public void keepAlive() {
		...
		try {
			...
			manager.keepAlive(id);
			this.getResponse().setStatus(Status.SUCCESS_NO_CONTENT,
					"DC successfully kept alive with its resources");
			...
			logger.error("Error while trying to keep dc alive", e);
			this.getResponse().setStatus(Status.SERVER_ERROR_INTERNAL,
					e.getMessage());
			this.getResponse().setEntity(
					"Error while trying to keep dc alive: " + e.getMessage(),
					MediaType.TEXT_PLAIN);
		} finally {
