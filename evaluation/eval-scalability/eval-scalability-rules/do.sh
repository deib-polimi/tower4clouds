...
if [[ $# -lt 6 ]] ; then
    echo "Usage: $0 #processes #threads server-address server-user server-ssh-key #rules"
    exit 1
fi
...
log "Number of rules = ${N_RULES}"
N_CLIENTS=$(($N_PROCESSES*$N_THREADS))
log "Number of clients = ${N_CLIENTS}"
RES_FOLDER=${WD}/results/$N_RULES/$N_CLIENTS
rm -rf $RES_FOLDER
mkdir -p $RES_FOLDER

log "Killing any still running data collector"
pkill java
...
do
	java -jar $WD/target/eval-scalability-rules-*-jar-with-dependencies.jar \
		-process $i \
		-threads $N_THREADS \
    ...
    <monitoringRule id="CPURule${i}"
        ...
        <actions>
            <action name="OutputMetric">
                ...
                <parameter name="metric">AverageCPU${i}</parameter>
...
EOF
	log "Sleeping before installing next rule"
	sleep $(echo $N_RULES | awk '{print (60/$1)}')
	i=$[$i+1]
done
log "All rules installed"

niceSleep 1200 "Waiting for data collectors to start sending"
...
grep -B1 'Execution Time' $RES_FOLDER/da.log \
	| sed -e ':a' -e 'N' -e '$!ba' -e 's/\n //g' \
	| grep -v -- "^--$" \
	| awk -F' ' '{print $1","$15}' \
	> $RES_FOLDER/exec-time.csv
