...
if [[ $# -lt 5 ]] ; then
    echo "Usage: $0 #processes #threads server-address server-user server-ssh-key"
    exit 1
fi
...
do
	java -jar $WD/target/eval-scalability-traffic-*-jar-with-dependencies.jar \
		-process $i \
		-threads $N_THREADS \
...
grep "Execution Time" $RES_FOLDER/da.log | awk -F' ' '{print $4}' > $RES_FOLDER/exec-time.csv
